#!/usr/bin/env python3
"""
ETL script for processing Hebrew morphology codes from the TEHMC file.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('etl_hebrew_morphology.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('etl_hebrew_morphology')

def create_tables(conn):
    """Create the necessary tables for Hebrew morphology codes if they don't exist."""
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS bible.hebrew_morphology_codes (
        ...
        # Create indexes
        cur.execute("""
            CREATE INDEX IF NOT EXISTS idx_hebrew_morphology_code
            ON bible.hebrew_morphology_codes (code)
        """)
        
        conn.commit()
        logger.info("Hebrew morphology tables created or already exist")

def parse_morphology_file(file_path):
    """
    Parse the TEHMC file to extract morphology codes and their explanations.
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            if brief_section_start >= 0:
                ...
                while i < len(lines) and (full_section_start == -1 or i < full_section_start):
                    ...
                    if re.match(r'^(H:|A:|N:|H[A-Z]|A[A-Z]|N[A-Z]|Ps|Pp|Os|Op|Ss|Sp)', line):
                        parts = line.split('\t')
                        if len(parts) >= 1:
                            code = parts[0].strip()
...
def load_morphology_data(db_connection, morphology_data):
    ...
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM bible.hebrew_morphology_codes")
        count = cur.fetchone()[0]
        
        if count > 0:
            logger.info(f"Found {count} existing Hebrew morphology codes. Truncating table to update with new data.")
            # Truncate the table to replace with new data
            cur.execute("TRUNCATE TABLE bible.hebrew_morphology_codes RESTART IDENTITY CASCADE")
        ...
        for item in morphology_data:
            cur.execute("""
                INSERT INTO bible.hebrew_morphology_codes
        ...
        db_connection.commit()
        logger.info(f"Inserted {inserted_count} Hebrew morphology codes")
        
    except Exception as e:
        db_connection.rollback()
        logger.error(f"Error loading Hebrew morphology data: {e}")
        raise e

def main(file_path):
    """Main ETL process for Hebrew morphology codes."""
    logger.info(f"Starting Hebrew morphology ETL process with file: {file_path}")
    ...
    try:
        ...
        logger.info("Hebrew morphology ETL process completed successfully")
        
    except Exception as e:
        logger.error(f"Error in Hebrew morphology ETL process: {e}")
        sys.exit(1)
    finally:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process Hebrew morphology codes from TEHMC file')
    parser.add_argument('--file', type=str, required=True, help='Path to the TEHMC file')
